(function($,undefined){
 ...
 $.fn.JSAjaxFileUploader = function(options) {
	 ...
	 var form = ['<form method="post" class="JSUploadForm" id="JSUploadForm" enctype="multipart/form-data" action='
	             ,settings.url
	             ,'>'
	             ,'<label class="JSFileChoos"><input type="file" multiple style="display:none"/>'
	             ,settings.inputText
	             ,'</label>'
	             ,(settings.autoSubmit ? '' : '<label class="JSFileCount"></label><a href="#" class="startJSuploadButton" >'+settings.uploadTest+'</a>'),'<ul class="JSpreveiw"></ul></form>'];
	 this.onChange = function(JSNode,files){
		for (var i = 0; i < files.length; i++){
			var file = files[i];
			var re = new RegExp('.('+settings.allowExt+')$', "i");
			var $JSView = $('#JSUploadForm ul.JSpreveiw',JSNode);
			elementId++;
			var oprIcons = ['<span class="JSOprIcons"><span class="JSOprIconsStop" id="stop',elementId,'" title="Stop Upload"></span><span class="JSOprIconsClose" id="close',elementId,'"  title="Close"></span></span>'];
			...
			if(!!file.type.match(/image.*/)) $.JSRender(JSNode,file,settings,elementId,oprIcons);
				...
				$('#JSUploadForm ul.JSpreveiw',JSNode).append(li);
				var data = {
					JSNode:JSNode,
					file:file,
					settings:settings,
		...
		if(settings.zoomPreview)
			$('#JSUploadForm ul li table td img.JSThumbnail',this).JSEnlargeImage({zoomWidth:settings.zoomWidth,zoomHeight:settings.zoomHeight});
	 };
	return this.each(function(){
		...
		if(settings.autoSubmit)
			$('#JSUploadForm input[type="file"]',JSNode).change(function(event){
			...
			$('#JSUploadForm .startJSuploadButton',JSNode).click(function(event){
 ...
 $.JSRender = function(JSNode,file,settings,id,oprIcons) {
     ...
     $('#JSUploadForm ul.JSpreveiw',JSNode).append(prepareLi(img.src,file,id,true,''));
     $('#JSUploadForm ul.JSpreveiw span#fileId'+(id)+'_spanPercent',JSNode).html('<p style="font-size:9px;margin:0px">Waiting...</p>');
     var data = {
        JSNode:JSNode,
     	file:file,
     	settings:settings,
...
var _upload = function(param){
	...
	if(formData){
		...
		var $stop = $('span#stop'+id,JSNode); 
		var $close = $('span#close'+id,JSNode);
		    ...
		    xhr.upload.addEventListener("progress", function(evt){
		      ...
		      if (settings.showProgress && evt.lengthComputable) {
		        ...
		        if(percentComplete >= 100){
		        	var $eml = $('#JSUploadForm ul.JSpreveiw li#JSfileLi'+(id),JSNode); 
...
function prepareLi(imgSrc,file,id,isUploading,errMsg){
	...
	if($(window).width() > 400){
		li = ['<li id="JSfileLi',id,'"><table style="width:100%"><tr><td rowspan=3 style="width: 60px;"><div style="width: 12px;text-align: center;"><img class="JSThumbnail" id="thimbImg',id,'" src="',imgSrc,'" width=50 height=50></div></td><td><span class="JSfilename">',_trimFileName(file.name,30),'</span><td>',
	          (isUploading ? ('<span class="JSOprIcons"><span class="JSOprIconsStop" id="stop'+id+'" title="Stop Upload">') : '' ),'</span><span class="JSOprIconsClose" id="close',id,'" index="',id,'"title="Close"></span></span></td></tr>',//end of 1st Row
