...
<%@page import="com.hefei.admin.cas.administrator.vo.AuthInfo"%>
...
<html >
  ...
  <body>
    	<nav class="breadcrumb">
    		...
    		<span class="c-gray en">&gt;</span> 用户授权
...
function submitAuth(){
	var changedCheckbox='';//id=1-check=Y_id=2-check=N
	...
	for(var i=0; i<ch.length; i++){
		...
		if(ch[i].checked == true){
			if(checkYes == oldCheckInfo){
				...
				changedCheckbox = changedCheckbox + "id="+roleId+"-check=Y";
			}
		}else{
			if(checkYes == oldCheckInfo){
				changedCheckbox = changedCheckbox + "id="+roleId+"-check=N";
			}else{
    ...
    if(changedCheckbox != ''){
    	console.log(changedCheckbox);
    	 $.ajax({  
   	      type: "POST",  
   	      async: false,  
   	      url: '<%=AdminConstants.ADMIN_SERVER_URL %>/cas/admin/authRole.do',
   	      dataType: "json",
   	      data:"adminId=" + $("#adminId").val() +"&idAndCheck=" + changedCheckbox,
   	      // dataType: "jsonp",
