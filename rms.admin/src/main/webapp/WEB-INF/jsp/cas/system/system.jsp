...
<link rel="stylesheet" type="text/css" href="<%=AdminConstants.STAIC_SERVER_URL %>/static/h-ui.admin/css/style.css" />
  ...
  <body>
	<article class="page-container">
    	...
    	<form action="<%=AdminConstants.ADMIN_SERVER_URL %>/cas/system/create.do" method="post" class="form form-horizontal" id="systemForm">
		  	<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>系统名称：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="" placeholder="" id="systemName" name="systemName"   nullmsg="请输入系统名称">
				</div>
			</div>
			<div class="row cl">
				<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
					<button type="button" class="btn btn-success radius" id="system-create" name="system-save"><i class="icon-ok"></i> 确定</button>
				</div>
			</div>
		</form> 
      </article>
  ...
  $(document).ready(function(){
	  $("#system-create").click(function(){systemCreateSubmit();});
  });
  function systemCreateSubmit(){
	  $.ajax({  
	      type: "POST",  
	      async: false,  
	      url: $('#systemForm').attr("action"),  
	      dataType: "json",
