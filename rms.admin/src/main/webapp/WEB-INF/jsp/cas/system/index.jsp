...
<html >
  ...
  <body>
    	...
    	<div class="page-container">
			<div class="text-c"> 
				<form method="post" action="<%=AdminConstants.ADMIN_SERVER_URL %>/cas/system/search.do">
					<input type="hidden" name="pageIndex" id="pageIndex">
					<input type="hidden" name="pageSize" id="pageSize">
					<input type="text" name="systemName" id="" placeholder="系统名称" style="width:250px" class="input-text">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20"> 
				<span class="l">
					...
					<a class="btn btn-primary radius" href="javascript:;" onclick="system_add('添加系统','<%=AdminConstants.ADMIN_SERVER_URL %>/cas/system/toCreate.do','800')"><i class="Hui-iconfont">&#xe600;</i>添加系统</a>
				</span>
			</div>
			<div class="mt-20">
					...
					<tbody>
						<c:if test="${not empty pagination.items }">
							...
							<tr class="text-c">
								...
								<td>${item.id }</td>
								<td class="text-l">${item.systemName }</td>
								<td>
									<c:if test="${item.delFlag eq SystemInfo.DEL_FLAG_NO }">有效</c:if>
									<c:if test="${item.delFlag eq SystemInfo.DEL_FLAG_YES }">无效</c:if>
								</td>
